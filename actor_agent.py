...
class ActorAgent(Agent):
    def __init__(self, sess, node_input_dim, job_input_dim, hid_dims, output_dim,
        ...
        logits = tf.log(self.node_act_probs)  # log取对数
    ...
    def get_gradients(self, node_inputs, job_inputs,
        ...
        return self.sess.run([self.act_gradients, [self.adv_loss, self.entropy_loss]],
                             feed_dict={i: d for i, d in zip(
                                 [self.node_inputs] + [self.job_inputs] + [self.node_valid_mask] + [self.job_valid_mask] +
                                 self.gcn.adj_mats + self.gcn.masks + self.gsn.summ_mats + [self.dag_summ_backward_map] + [self.node_act_vec] +
                                 [self.job_act_vec] + [self.adv] + [self.entropy_weight],
                                 [node_inputs] + [job_inputs] + [node_valid_mask] + [job_valid_mask] +
                                 gcn_mats + gcn_masks + [summ_mats, running_dags_mat] +
                                 [dag_summ_backward_map] + [node_act_vec] + [job_act_vec] + [adv] + [entropy_weight])
                                        }
                             )
